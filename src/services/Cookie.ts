...
export default class CookieService {
  public static async createSessionCookie(realmName: string, username: string, password: string): Promise<string> {
    const tokens = await KeycloakApi.login(realmName, username, password);
    return CryptoService.encrypt(tokens);
  }

  public static async renewSessionCookie(realmName: string, sessionCookie: string): Promise<string> {
